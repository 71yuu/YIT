...
import com.yuu.yit.education.user.common.bean.qo.SendSmsLogQO;
import com.yuu.yit.education.user.common.bean.vo.SendSmsLogVO;
...
public class BossSendSmsLogBiz {
	...
	public int send(SendSmsLogQO qo) {
		...
		try {
			...
			if (result) {
				// 成功发送，验证码存入缓存：5分钟有效
				redisTemplate.opsForValue().set(qo.getMobile(), sendSmsLog.getSmsCode(), 5, TimeUnit.MINUTES);
